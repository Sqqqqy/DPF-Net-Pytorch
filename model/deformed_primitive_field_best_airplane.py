...
class DPF(nn.Module):
    def __init__(self, num_template=16, z_dim=256, part_dim=64, is_stage2=False):
        super().__init__()
        self.num_template = num_template
        self.z_dim = z_dim
        self.part_dim = part_dim
        ...
        self.conv_scale = nn.Conv1d(128, 3, kernel_size=1, bias=True)
        # self.conv_scale.bias.data = torch.Tensor([0.9,0.9,0.9])
        ...
        self.conv_density = nn.Sequential(nn.Conv1d(64, 32, kernel_size=1, bias=True),
                                        nn.LeakyReLU(negative_slope=0.2, inplace = True),
    ...
    def primitive_deform(self, point_coord, z_fusion, rotation_, scale_, loc_, density_):
        ...
        for split_i in range(self.num_template):
            ...
            correction_reg_loss = correction_reg_loss + torch.abs(exist_part).sum(-2)
        
        G_ = torch.cat(G_, dim=-1) # [bs, N, num_temp]
       
        deform_loss = (deform_reg_loss.mean()*10  + correction_reg_loss.mean()*10) / self.num_template
    ...
    def forward(self, point_voxels, point_coord, apply_grad_loss=False):
        ...
        density_ = torch.sigmoid(self.conv_density(ins_codes.transpose(2, 1)).transpose(2, 1))
        ...
        return {'G_': G_,
                ...
                'part_codes_loss': ((F.normalize(q_,dim=-1,p=2)-torch.Tensor([1,0,0,0]).to(ctx))**2).sum(-1).mean(),#+0.01*((torch.abs(com_norm_code @ (com_norm_code.transpose(2,1)))-self.num_template)**2).sum(-1).mean(),
