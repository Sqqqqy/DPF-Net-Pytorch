...
from model.k_deformer_split_corse2fine import k_deformer
...
class k_generator(nn.Module):
    ...
    def forward(self, points, z, part_codes, deformations):
        ...
        for part_i in range(self.num_template):
            z_fusion = self.embed_layer3(embed_zs[part_i] + (embed_part[:,part_i,:].unsqueeze(1)))
...
class fine_generator(nn.Module):
    def __init__(self, gf_dim=256,gf_split=8,z_dim=512,num_template=4, common_dim=128):
        ...
        self.common_dim = common_dim
        self.num_template = num_template
        self.layer1 = nn.Linear(self.gf_dim*self.num_template, self.gf_dim*4)
...
class BAE_k_deform_k_bae_corse2fine(nn.Module):
