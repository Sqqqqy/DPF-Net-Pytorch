...
class k_deformer(nn.Module):
    def __init__(self, hidden_size=128, z_dim=512, part_dim=128, num_template=4, common_dim=96):
        ...
        self.embed_layer = nn.Sequential(nn.Linear(self.z_dim, self.hidden_dim),
                                          nn.Linear(self.hidden_dim, self.hidden_dim),
                                          nn.Linear(self.hidden_dim, self.num_template*self.part_dim),)
        # nn.Linear(self.z_dim, self.num_template*self.part_dim)
    ...
    def forward(self, points, z, part_poses):
        bs = points.shape[0]
        num_points = points.shape[1]
        embeddings = self.embed_layer(self.global_feature)
        part_codes = embeddings.reshape(1, self.num_template, self.part_dim)
        ...
        for split_i in range(self.num_template):
            ...
            # affine_deform = torch.repeat_interleave(part_poses[:,split_i,:].unsqueeze(1),num_points, dim=1)
            # deform_part += affine_deform
