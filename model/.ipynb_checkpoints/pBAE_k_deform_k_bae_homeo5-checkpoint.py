...
# v3 follow spaghetti模型中decompositionNetwork的设计
# v3 的实验说明，这样的模型架构没问题，但是会存在部分homeo没有学习成part

# v4 follow neural parts中模型的scale factor 和 translation factor的设计
# 为每个homeo添加位置信息，目的是降低deformer学习homeo的复杂程度（与位置尽可能无关）

# v5 之前的deformation值域在实数范围内可能过大，将其限制在0，1之间。目的是让deformation更有意义
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from model.k_deformer_split_homeo5 import k_deformer
...
class k_generator(nn.Module):
    ...
    def forward(self, points, ins_codes, com_codes, deformations, scale_codes, trans_codes):
        bs = points.size(0)
        ...
        for split_i in range(self.num_template):
            ...
            #scale_codes_i [bs, 1, 3] trans_codes_i [bs, 1, 3] deformation_i [bs, N, 3]
            deformation_i = deformations[split_i]
            deformation_i = deformation_i*scale_codes[:,split_i,:].unsqueeze(1)+trans_codes[:,split_i,:].unsqueeze(1)
            
            Gs_.append(self.BAEs[split_i](points, deformation_i))
...
class BAE_k_deform_k_bae_homeo5(nn.Module):
    def __init__(self, num_template=6, z_dim=256, part_dim=64, common_dim=96):
        ...
        self.split_layer = nn.Linear(self.z_dim, self.num_template*self.part_dim)
        
        self.part_mlp = nn.Sequential(nn.Linear(self.part_dim, self.hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(self.hidden_dim, self.hidden_dim),
                                        nn.ReLU(),
                                        nn.Linear(self.hidden_dim, self.part_dim),)
        self.map_s = nn.Sequential(
                nn.Linear(self.part_dim, self.hidden_dim),
                nn.ReLU(),
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.ReLU(),
    ...
    def forward(self, fps_point, point_coord):
        ...
        ins_codes = self.split_layer(instance_feat).reshape(bs, self.num_template, self.part_dim)
        com_codes = self.split_layer(self.global_feature).reshape(1, self.num_template, self.part_dim)
        
        ins_codes = self.part_mlp(ins_codes)
        com_codes = self.part_mlp(com_codes)
        
        scale_codes = self.map_s(ins_codes)
        trans_codes = self.map_t(ins_codes)-0.5
        
        deform_out = self.deformer(point_coord, ins_codes, com_codes)
        
        G_, G = self.generator(point_coord, ins_codes, com_codes, deform_out, scale_codes, trans_codes)
