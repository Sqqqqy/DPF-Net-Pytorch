...
from model.k_deformer_fusion_split import k_deformer
...
class k_generator(nn.Module):
    def __init__(self, gf_dim=256,gf_split=8, z_dim=512, num_template=4, common_dim=128, part_dim=64):
        ...
        # self.embed_layer1 = nn.Linear(self.z_dim, self.num_template*self.part_dim)
    ...
    def forward(self, points, z, part_codes, deformations):
        ...
        embed_part = part_codes
        # embed_z = self.embed_layer1(z)
        # embed_z = F.leaky_relu(embed_z, negative_slope=0.01, inplace=True)
        # embed_zs = torch.split(embed_z,self.part_dim,dim=2)
        # embed_part = self.embed_layer2(part_codes)
        # embed_part = F.leaky_relu(embed_part, negative_slope=0.01, inplace=True)
        Gs_ = []
        for part_i in range(self.num_template):
            # z_fusion = torch.cat([embed_zs[part_i], (embed_part[:,part_i,:].unsqueeze(1))], dim=-1)
            z_fusion = self.embed_layer3((embed_part[:,part_i,:].unsqueeze(1)))
            Gs_.append(self.BAEs[part_i](points, z_fusion, deformations[part_i]))
...
class generator(nn.Module):
    ...
    def forward(self, points, z, deformations):
        ...
        deformation = deformations[:,:,:3]
        
        delta = deformations[:,:,3]
...
class BAE_k_deform_k_bae_fusion(nn.Module):
    ...
    def forward(self, fps_point, point_coord):
        ...
        G_, G = self.generator(point_coord, global_feat.reshape(bs,1,-1), part_codes.reshape(bs,self.num_template,self.part_dim), deform_out)
