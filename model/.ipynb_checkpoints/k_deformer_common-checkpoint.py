...
class part_classifier(nn.Module):
    def __init__(self, hidden_size=128, part_z_dim=32, common_z_dim=96, num_template=4):
        super().__init__()

        self.activation = nn.ReLU()
        self.hidden_size = hidden_size
        self.part_z_dim = part_z_dim
        self.common_z_dim = common_z_dim
        self.num_template = num_template
        self.h1 = nn.Linear(self.part_z_dim+self.common_z_dim, self.hidden_size)
        self.h2 = nn.Linear(self.hidden_size,self.hidden_size)    
        self.h3 = nn.Linear(self.hidden_size,self.num_template)
    ...
    def forward(self, instance_z, common_z):
        instance_z = instance_z.reshape((1,1,-1))
        common_z = common_z.reshape((1,1,-1))
...
class k_deformer(nn.Module):
    def __init__(self, hidden_size=128, z_dim=512, part_dim=128, num_template=4, common_dim=96):
        ...
        self.classifier = part_classifier(part_z_dim=self.part_dim, common_z_dim=self.common_dim)
    ...
    def forward(self, points, z):
        ...
        for split_i in range(self.num_template):
            predict_cls.append(self.classifier(self.part_feature[split_i], self.common_feature))
        ...
        return deformations, predict_cls