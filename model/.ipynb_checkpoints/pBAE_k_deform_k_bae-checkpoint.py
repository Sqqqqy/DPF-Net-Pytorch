...
class k_generator(nn.Module):
    def __init__(self, gf_dim=256,gf_split=8, z_dim=512, num_template=4, common_dim=128, part_dim=64):
        ...
        self.embed_layer = nn.Linear(self.part_dim, self.part_dim)
    ...
    def forward(self, points, part_zs, part_codes, deformations):
        bs = points.size(0)
        ...
        for part_i in range(self.num_template):
            z_fusion = self.embed_layer((part_codes[:,part_i,:].unsqueeze(1)) + (part_zs[:,part_i,:].unsqueeze(1)))
            corse_out, fine_out_embed = self.BAEs[part_i](points, z_fusion, deformations[part_i])
...
class BAE_k_deform_k_bae(nn.Module):
    ...
    def forward(self, fps_point, point_coord):
        ...
        deform_out, part_codes = self.deformer(point_coord, embed_zs, part_poses)
        G_, G_corse, G_fine = self.generator(point_coord, embed_zs.reshape(bs,self.num_template,self.part_dim), part_codes.reshape(1,self.num_template,self.part_dim), deform_out)
