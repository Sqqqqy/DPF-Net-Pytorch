...
class generator(nn.Module):
    ...
    def forward(self, points, z, deformations, delta):
        ...
        out = torch.max(h3_outs, axis=2, keepdims=True)

        return h3_outs, out.values
...
class BAE_deform(nn.Module):
    def __init__(self, num_template=3):
        super().__init__()
        self.num_template = num_template
        self.encoder = encoder()
        self.generator = generator(num_template=self.num_template)
        self.deformer = k_deformer(num_template=self.num_template)
        
    def forward(self, fps_point, point_coord):
        bs, N = point_coord.shape[0], point_coord.shape[1]
        E = self.encoder(fps_point)
        deform_out = self.deformer(point_coord, E)
        deformations = deform_out[:,:,:-1].reshape(bs, N, self.num_template, 3)
        delta = deform_out[:,:,-1]

        G_, G = self.generator(point_coord, E.reshape(bs,1,-1), deformations, delta)
