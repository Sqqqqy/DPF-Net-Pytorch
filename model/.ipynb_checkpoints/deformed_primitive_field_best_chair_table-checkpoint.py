...
class DPF_best(nn.Module):
    def __init__(self, num_template=8, z_dim=256, part_dim=64, is_stage2=False, primitive_type='cuboid'):
        ...
        self.conv_scale = nn.Conv1d(128, 6, kernel_size=1, bias=False)
        ...
        if self.stage2:
            ...
            # self.mixing_network = Transformer(self.part_dim*2, self.num_template, 1,
            #                                                   act=nn.functional.leaky_relu, norm_layer=nn.LayerNorm)
    ...
    # def primitive_space(self, rotation, scale, points, p_type):
    #     primitive_G_ = []
    #     # p_type_dict = {0:'bbox', 1:'cylinder', 11:'ellipsoid', 2:'sphere'}
    #     for tt in ['bbox', 'cylinder']
    #         occupancy = self.prob_field(rotation, scale, points, tt)
    #         primitive_G_.append(occupancy)

    #     primitive_G_ = torch.cat(primitive_G_, dim=-1) # [bs, N, num_temp]
    #     return primitive_G
    ...
    def primitive_deform(self, point_coord, z_fusion, rotation_, scale_, loc_, density_):
        ...
        p_type_dict = {0:'bbox', 1:'cylinder', 2:'ellipsoid', 3:'sphere'}
    ...
    def forward(self, point_voxels, point_coord, is_training=False, apply_grad_loss=False):
        ...
        return {'G_': G_sampled,
                ...
                'part_codes_loss': ((F.normalize(q_,dim=-1,p=2)-torch.Tensor([1,0,0,0]).to(ctx))**2).mean(), #.sum(-1).mean(),#+0.01*((torch.abs(com_norm_code @ (com_norm_code.transpose(2,1)))-self.num_template)**2).sum(-1).mean(),
