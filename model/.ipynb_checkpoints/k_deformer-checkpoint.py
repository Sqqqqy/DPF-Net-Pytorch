...
class deformer(nn.Module):
    def __init__(self, hidden_size=128, z_dim=256, part_dim=64):
        ...
        self.h1 = nn.Linear(3+self.z_dim+self.part_dim, self.hidden_size)
    ...
    def forward(self, points, z, instance_z):
        bs = points.shape[0]
...
class k_deformer(nn.Module):
    def __init__(self, hidden_size=128, z_dim=512, part_dim=128, num_template=4, common_dim=96):
        ...
        self.common_dim = common_dim
        self.deformer = deformer(z_dim=self.z_dim, part_dim=self.part_dim)
    ...
    def forward(self, points, z):
        ...
        for split_i in range(self.num_template):
            deform_part = self.deformer(points, z, self.part_feature[split_i])
            if split_i == 0:
