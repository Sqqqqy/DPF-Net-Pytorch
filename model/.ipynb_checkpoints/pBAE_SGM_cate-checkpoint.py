...
from model.pDecoder import pointDecoder
...
class BAE_sgm_cate(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = encoder()
        self.generator = generator()
        self.cate_generator = pointDecoder()
    def forward(self, fps_point, point_coord):
        bs = fps_point.shape[0]
        E = self.encoder(fps_point)
        G_ = self.generator(point_coord, E.reshape(bs,1,-1))
        G_cate = self.cate_generator(point_coord)
        G_ = G_ * G_cate
        G = torch.max(G_, axis=2, keepdims=True).values
        return {'G_': G_,
                'G': G}