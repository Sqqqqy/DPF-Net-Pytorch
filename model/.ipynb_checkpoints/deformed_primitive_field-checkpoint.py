...
class DPFNet(nn.Module):
    ...
    def primitive_deform(self, point_coord, z_fusion, rotation_, scale_, loc_, density_):
        ...
        for split_i in range(self.num_template):
            ...
            # p_type = p_type_dict[split_i%2]
            p_type = self.primitive_type
            
            primitive_coord = point_coord - loc_[:,split_i,:].unsqueeze(1)
            occupancy = self.querying_occupancy(p_type, primitive_coord, rotation_[:,split_i,:,:].unsqueeze(1), scale_[:,split_i,:].unsqueeze(1), loc_[:,split_i,:].unsqueeze(1), density_[:,split_i,:].unsqueeze(1))
            
            deform_input = torch.cat([occupancy, point_coord], dim=-1)
            G_structure.append(occupancy)
...
# 
        ...
        # deform_loss = (deform_reg_loss.mean()*10 + correction_reg_loss.mean()*50/10) / self.num_template
