...
# homeo -> 把instance code给deformer，希望他能根据instance信息出不同的坐标映射结果
# this -> 去掉deformer, 只用k个bae，验证是否依然能够将part分开
# 如果能够做到和之前差不多的结果，我们接下来需要对deformer进一步设计/或者以此来设计更好的框架

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from model.k_deformer_split_homeo2 import k_deformer
...
class k_generator(nn.Module):
    ...
    def forward(self, points, ins_codes, com_codes):
        ...
        for split_i in range(self.num_template):
            z_fusion = self.fusion_layer(ins_codes[:,split_i,:].unsqueeze(1)+com_codes[:,split_i,:].unsqueeze(1))
            Gs_.append(self.BAEs[split_i](points, z_fusion))
...
class generator(nn.Module):
    def __init__(self, gf_dim=256,gf_split=8,z_dim=512,num_template=4, common_dim=128):
        ...
        self.common_dim = common_dim
        self.num_template = num_template
        self.layer1 = nn.Linear(3+self.z_dim, self.gf_dim*4)
...
class BAE_k_bae(nn.Module):
    ...
    def forward(self, fps_point, point_coord):
        ...
        G_, G = self.generator(point_coord, ins_codes, com_codes)
